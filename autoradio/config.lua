...
Config.DistanceToVolume = 20.0 -- The distance that will be with the volume at 1.0, so if the volume is 0.5 the distance will be 15.0, if the volume is 0.2 the distance will be 6.
...
Config.CommandVehicle = "autoradio" -- Only will work if Config.ItemInVehicle == false
...
Config.Zones = {
	...
	{
		name = "Beach Party", -- The name of the radio, it doesn't matter
		coords = vector3(-1520.14,-1495.35,6.24), -- the position where the music is played
		...
		loop = false,-- when does the music stop it will repaeat?
		deftime = 0, -- where does the music starts? 0 and it will start in the beginning
		changemusicblip = vector3(-1518.86, -1476.81, 6.84) -- where the player can change the music
	},
}